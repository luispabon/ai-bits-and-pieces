...
6. Display the `overview.md` contents to the user in the console immediately after writing it. Do not treat the file as an internal artifact only.
...
- `overview.md` must be shown to the user and discussed before `plan.yaml` is created.

## Overview Checkpoint

After writing `overview.md`:

1. Show its contents in the console.
...
Do not create `plan.yaml` in the same turn unless the user explicitly asks to continue planning after seeing the overview.
...
- Present the overview first in the console, not just on disk, and hold back detailed implementation until the user wants it.
